...
def face_and_blink_detection(frame, gray, shared_state, detector, predictor, overlay):
    ...
    for face in shared_state.dlib_faces:
        ...
        if np.mean(shared_state.is_centered_queue) < 0.90 or abs(np.mean(shared_state.up_down_history)) > 0.4 or abs(
                np.mean(shared_state.left_right_history)) > 0.4:
...
def main():
    ...
    while True:
        ...
        """
        # Create threads
        t1 = threading.Thread(target=face_and_blink_detection,
                              args=(frame, gray, shared_state, detector, predictor, overlay))
        ...
        face_and_blink_detection(frame, gray, shared_state, detector, predictor, overlay)
        pose_estimation_and_shake_nod_detection(frame, shared_state, fa, overlay)
