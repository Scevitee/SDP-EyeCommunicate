...
class Overlay(QWidget):
    ...
    def toggle_pin(self):
        if self.is_pinned:
            ...
            # Adjust margins
            padding = 20  # 20 pixels padding on each side from the center
            self.layout.setContentsMargins(0, 0, 0, 0)

            # Center the eyeball button
            self.layout.setAlignment(self.eyeball_button, Qt.AlignCenter)

            # Resize window to fit the eyeball button plus padding
            self.eyeball_button.adjustSize()
            btn_size = self.eyeball_button.size()
            window_width = btn_size.width() + padding * 2
            window_height = btn_size.height() + padding * 2
    ...
    def update_interactable_widgets(self):
        ...
        for button in [self.toggle_button, self.close_button, self.home_button, self.drawing_button, self.keyboard_button]:
            if button.isVisible():
...
class DraggableButton(QPushButton):
    ...
    def leaveEvent(self, event):
        # Revert style when not hovering
        self.setStyleSheet(self.default_style)
