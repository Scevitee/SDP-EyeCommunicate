...
class PenIndicator(QWidget):
    ...
    def paintEvent(self, event):
        ...
        grey_color = QColor(Qt.lightGray)
...
class DrawingCanvas(QWidget):
    def __init__(self, color_indicator):
        ...
        self.colors = [
            QColor(Qt.black), QColor(Qt.white), QColor(Qt.red), QColor(Qt.green), QColor(Qt.blue),
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if event.modifiers() & Qt.ShiftModifier:
                # Perform flood fill
                self.save_to_undo()
                point = self.map_to_image(event.pos())
...
class ArtWidget(QWidget):
    ...
    def generate_image(self):
        # Hide loading label and prompt widget in case they are visible
